...
try {
    ...
    } else if (typeof DESTINOS !== 'undefined') {
        appDestinos = DESTINOS;
    }
} catch (e) {
    console.warn("Error initializing appDestinos from cache/local:", e);
    if (typeof DESTINOS !== 'undefined') appDestinos = DESTINOS;
}
...
try {
    ...
    } else if (typeof EVENTOS !== 'undefined') {
        appEventos = EVENTOS;
    }
} catch (e) {
    console.warn("Error initializing appEventos from cache/local:", e);
    if (typeof EVENTOS !== 'undefined') appEventos = EVENTOS;
}
    ...
    try {
        ...
        if (cachedDest) {
            try {
                ...
                console.error("Error al parsear caché de destinos:", err);
            }
        }
        if (!appDestinos || appDestinos.length === 0) {
            try {
                if (typeof DESTINOS !== 'undefined') {
                    appDestinos = DESTINOS;
                } else {
                    console.log("DESTINOS no definido en fallback. Intentando fetch destinos.json...");
                    const res = await fetch('destinos.json');
                    if (res.ok) {
                        appDestinos = await res.json();
                        console.log(`destinos.json cargado dinámicamente con éxito (${appDestinos.length} destinos).`);
                    }
                }
            } catch (err) {
                console.error("Error en fallback definitivo de destinos:", err);
            }
        }
        ...
        if (cachedEv) {
            try {
                ...
                console.error("Error al parsear caché de eventos:", err);
            }
        }
        if (!appEventos || appEventos.length === 0) {
            try {
                if (typeof EVENTOS !== 'undefined') {
                    appEventos = EVENTOS;
                } else {
                    console.log("EVENTOS no definido en fallback. Intentando fetch eventos.json...");
                    const res = await fetch('eventos.json');
                    if (res.ok) {
                        appEventos = await res.json();
                        console.log(`eventos.json cargado dinámicamente con éxito (${appEventos.length} eventos).`);
                    }
                }
            } catch (err) {
                console.error("Error en fallback definitivo de eventos:", err);
            }
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Load and sync updated data from Google Sheets in the background asynchronously
    loadData();
});
...
// Show the loading transition view before displaying search results
function showTransitionAd() {
    // 1. Switch to the loading tab
